...
const ServerSidebar = ({ onServerSelect, selectedServerId, servers = [], onServerUpdate, onDiscoverClick, onHomeClick }) => {
    ...
    const [isCreating, setIsCreating] = useState(false); // Action Loader
                    ...
                    <div className="bg-[#1e1e24] p-8 rounded-2xl w-[600px] shadow-2xl border border-white/10 relative overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="flex justify-between items-center bg-[#17171c] -mx-8 -mb-8 p-4 px-8 mt-4">
                            ...
                            <button
                                ...
                                disabled={isCreating}
                                className={`px-8 py-2.5 bg-rose-600 hover:bg-rose-500 text-white rounded-lg font-bold shadow-lg shadow-rose-900/20 transition-all transform hover:scale-105 flex items-center justify-center gap-2 ${isCreating ? 'opacity-70 cursor-wait' : ''}`}
                            >
                                {isCreating && (
